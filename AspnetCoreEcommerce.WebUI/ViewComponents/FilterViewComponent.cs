...
namespace AspnetCoreEcommerce.WebUI.ViewComponents
{
    ...
    public class FilterViewComponent: ViewComponent
    {
        ...
        public IViewComponentResult Invoke(string nameFilter = "", string categoryFilter = "",
            string manufacturerFilter = "")
        {
            ...
            foreach (var category in allCategories)
            {
                ...
                if (manufacturerFilter.Length > 0)
                {
                    qty = filterModel.Quantity = _productService.Table()
                        ...
                        .Where(x => x.Manufacturers.Any(m =>
                            m.Manufacturer.Name.ToLower() == manufacturerFilter.ToLower()))
                        .Where(x => x.Categories.Any(c => c.Category.SeoUrl.ToLower() == category.SeoUrl.ToLower()))
                        .Where(x => x.Published == true)
                        .AsNoTracking()
                        .Count();
            ...
            var allManufacturer = _manufacturerService.GetManufacturers();
            ...
            foreach (var manufacturer in allManufacturer)
            {
                ...
                if (nameFilter.Length > 0)
                {
                    qty = filterModel.Quantity = _productService.Table()
                        ...
                        .Include(x => x.Manufacturers).ThenInclude(x => x.Manufacturer)
                        .Where(x => x.Manufacturers.Any(m =>
                            m.Manufacturer.Name.ToLower() == manufacturer.Name.ToLower()))
