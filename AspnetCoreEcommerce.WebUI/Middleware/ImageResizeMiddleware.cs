...
namespace AspnetCoreEcommerce.WebUI.Middleware
{
    public class ImageResizeMiddleware
    {
        ...
        {
            var imageExtension = new List<string> { "jpg", "png" };
            bool isImage = imageExtension.Any(ext => context.Request.Path.ToString().ToLower().EndsWith(ext));
            ...
            if (isImage)
            {
                ...
                //check if the image request have a query string
                ...
                if (context.Request.QueryString.HasValue)
                {
                    ...
                    if (w > 0 || h > 0)
                    {
                        using (var input = System.IO.File.OpenRead(imagePath))
                        {
                            ...
                            var scaledBitmap = bitmap.Resize(new SKImageInfo(w, h), SKFilterQuality.High);
    ...
    public static class ImageResizedMiddlewareExtensions
    {
